...
def get_average_sentence_length(text):
    '''
    Return the average length of a sentence in the text
    '''
    #create a list of all the sentences in a text, called sentences_in_text
    new_text = text.replace('?','.').replace('!','.')
    sentences_in_text = new_text.split('.')
    # .split returns a list of substrings (list of sentences)
    

#break up each sentences_in_text(a list) into a list of words (words_list) 
#save len(words_list) to new list containing sentence lengths (sentence_lengths)

    sentence_lengths = []
    
    for sentence in sentences_in_text:
        words_list = sentence.split()
        sentence_lengths.append(len(words_list))
 
    #take the average of all sentence lengths 
    #sum all the items in the list sentence_lengths (make a new list and iterate through each element, appending)                            
    sum_sentence_lengths = 0
                            
    for element in sentence_lengths:
        sum_sentence_lengths += element
    return sum_sentence_lengths / len(sentence_lengths)


get_average_sentence_length("Oh, hi! What are you up to? Let's get out of here!")  
...
class TextSample:
    ...
    def __repr__(self):
        return "Author: {author} \n Average Sentence Length: {average_sentence_length}".format(author = self.author, average_sentence_length=self.average_sentence_length)
...
#make uppercase letters into lowercase    
    ...
    all_lower_text = no_punc_text.lower()      
 #remove whitespace
    clean_text = all_lower_text.strip().split()
    return clean_text
...
def build_frequency_table(corpus):
    #corpus is a list
    #create a dictionary called frequency_table (list comprehension)
    frequency_table = dict((x,corpus.count(x)) for x in set(corpus))
...
def ngram_creator(text_list):
    '''
    Return a list of all adjacent pairs of words (as strings with a space in the centre).
    '''
    list_ngrams = []
    for i in range(len(text_list) - 1): 
        list_ngrams.append(text_list[i] + " " + text_list[i + 1])
    return list_ngrams
...
frequency_comparison({'do': 1, 'you': 2, 'see': 2, 'what': 1, 'i': 2}, {'do': 2, 'you': 2, 'see': 1, 'what': 3, 'i': 1})

    #iterate through table1's keys
    #check if table2 has the same key defined
...
def find_text_similarity(text_sample1, text_sample2):
    '''
    compare two text samples and return a float between 0 and 1, where 0 means completely different and 1 means the two samples are exactly the same.
    '''
...
print(lily_sample.author + '\'s similarity score is: ' + str(lily_similarity))
print(myrtle_sample.author + '\'s similarity score is: ' + str(myrtle_similarity))
print(gregg_sample.author + '\'s similarity score is: ' + str(gregg_similarity))
...
print(gregg_sample.author + " is the murderer!")
